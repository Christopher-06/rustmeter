...
pub fn flash_and_monitor_chip(
    ...
    match tool {
        ...
        FlashingTool::ProbeRs => {
            ...
            let rtt_listener = RttListener::new(session.clone())?;
            Ok(Box::new(rtt_listener))
        }
        FlashingTool::Auto => {
            // Choose default tool based on chip name
            if chip.to_lowercase().starts_with("esp32") {
                flash_and_monitor_chip(chip, FlashingTool::Espflash, elf_path)
            } else {
                flash_and_monitor_chip(chip, FlashingTool::ProbeRs, elf_path)
            }
        }
    }
}
